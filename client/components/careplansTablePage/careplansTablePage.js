...
Router.map(function(){
  this.route('careplansTablePage', {
    path: '/table/careplans',
    template: 'careplansTablePage'
  });
});
...
Template.careplansTablePage.events({
  'click .checkbox': function(event, template){
    event.stopPropagation();
    if(this.checked){
      CarePlans.update({_id: this._id}, {$set: {checked: false}});
    }else{
      CarePlans.update({_id: this._id}, {$set: {checked: true}});
    }
  },
  'click .flag': function(event, template){
    event.stopPropagation();
    if(this.flagged){
      CarePlans.update({_id: this._id}, {$set: {flagged: false}});
    }else{
      CarePlans.update({_id: this._id}, {$set: {flagged: true}});
    }
  },
...
Template.careplansTablePage.helpers({
  getName: function(){
...
Template.careplansTablePage.rendered = function(){
  Template.appLayout.layout();

  // step A:  initialize the table sorting functionality
  $(this.find('#careplansTable')).tablesorter();
